...
export default function RadioPlayer() {
  ...
  // Keep screen alive while playing to prevent the OS from backgrounding the app
  useKeepAwake();
  ...
  const pauseStream = useCallback(async () => {
    try {
      ...
      // Use pause() not stop() — stop() tears down the RNTP player session entirely,
      // which forces a full re-buffer when the user resumes or when AirPlay reconnects.
      // pause() keeps the session alive so switching back is near-instant.
  ...
  const togglePlay = useCallback(() => {
    ...
    if (playerState === 'playing') {
      ...
      // Resume the existing session — no rebuffering, no reset
      const player = audioPlayerRef.current;
      if (player) {
        setPlayerState('loading');
        player.resume()
          ...
          .catch(() => {
            // Session may have been lost (e.g. app killed in background) — full restart
            void startStream();
          });
      } else {
        void startStream();
